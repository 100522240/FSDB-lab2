...
round(
    (count(case when l.return is null then 1 else 0 end) / count(l.signature)) * 100, 2
) as percentage_unreturned loans
from drivers d
left outer join assign_drv a on d.passport = a.passport
left outer join services s on a.passport = s.passport
left outer join loans l on l.taskdate = s.taskdate, l.town = s.town, l.province = s.province
